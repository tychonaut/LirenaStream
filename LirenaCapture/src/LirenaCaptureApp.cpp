...
int main(int argc, char **argv)
{
	...
	gtk_init(&argc, &argv);
	//create widgets
	appPtr->localDisplayCtrl.widgets.controlWindow = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	appPtr->localDisplayCtrl.widgets.boxmain = gtk_vbox_new(FALSE, 0);
	appPtr->localDisplayCtrl.widgets.boxgpi = gtk_hbox_new(TRUE, 0);
	appPtr->localDisplayCtrl.widgets.boxx = gtk_hbox_new(FALSE, 0);
	appPtr->localDisplayCtrl.widgets.boxy = gtk_hbox_new(FALSE, 0);
	appPtr->localDisplayCtrl.widgets.labelexp = gtk_label_new("Exposure (ms)");
	appPtr->localDisplayCtrl.widgets.labelgain = gtk_label_new("Gain (dB)");
	appPtr->localDisplayCtrl.widgets.labelx0 = gtk_label_new("x0");
	appPtr->localDisplayCtrl.widgets.labelcx = gtk_label_new("cx");
	appPtr->localDisplayCtrl.widgets.labely0 = gtk_label_new("y0");
	appPtr->localDisplayCtrl.widgets.labelcy = gtk_label_new("cy");
	appPtr->localDisplayCtrl.widgets.gpi1 = gtk_check_button_new_with_label("GPI1");
	appPtr->localDisplayCtrl.widgets.gpi2 = gtk_check_button_new_with_label("GPI2");
	appPtr->localDisplayCtrl.widgets.gpi3 = gtk_check_button_new_with_label("GPI3");
	appPtr->localDisplayCtrl.widgets.gpi4 = gtk_check_button_new_with_label("GPI4");
	appPtr->localDisplayCtrl.widgets.exp = gtk_hscale_new_with_range(1, 1000, 1);
	appPtr->localDisplayCtrl.widgets.gain = gtk_hscale_new_with_range(0, 1, 0.1);	 //use dummy limits
	appPtr->localDisplayCtrl.widgets.x0 = gtk_spin_button_new_with_range(0, 128, 2); //use dummy max limit
	appPtr->localDisplayCtrl.widgets.y0 = gtk_spin_button_new_with_range(0, 128, 2); //use dummy max limit
	appPtr->localDisplayCtrl.widgets.cx = gtk_spin_button_new_with_range(4, 128, 4); //use dummy max limit
	appPtr->localDisplayCtrl.widgets.cy = gtk_spin_button_new_with_range(2, 128, 2); //use dummy max limit
	appPtr->localDisplayCtrl.widgets.raw = gtk_toggle_button_new_with_label("Display RAW data");
	appPtr->localDisplayCtrl.widgets.show = gtk_toggle_button_new_with_label("Live view");
	appPtr->localDisplayCtrl.widgets.run = gtk_toggle_button_new_with_label("Acquisition");
	//tune them
	gtk_window_set_title(GTK_WINDOW(appPtr->localDisplayCtrl.widgets.controlWindow), "streamViewer control");
	gtk_window_set_keep_above(GTK_WINDOW(appPtr->localDisplayCtrl.widgets.controlWindow), TRUE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(appPtr->localDisplayCtrl.widgets.raw), TRUE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(appPtr->localDisplayCtrl.widgets.show), TRUE);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(appPtr->localDisplayCtrl.widgets.run), TRUE); //actual start is delayed by 100ms, see below
	gtk_widget_set_sensitive(appPtr->localDisplayCtrl.widgets.boxgpi, FALSE);
	gtk_scale_set_digits(GTK_SCALE(appPtr->localDisplayCtrl.widgets.exp), 0);
	gtk_range_set_update_policy(GTK_RANGE(appPtr->localDisplayCtrl.widgets.exp), GTK_UPDATE_DISCONTINUOUS);
	gtk_range_set_update_policy(GTK_RANGE(appPtr->localDisplayCtrl.widgets.gain), GTK_UPDATE_DISCONTINUOUS);
	gtk_adjustment_set_value(gtk_range_get_adjustment(GTK_RANGE(appPtr->localDisplayCtrl.widgets.exp)), 10);
	gtk_scale_set_value_pos(GTK_SCALE(appPtr->localDisplayCtrl.widgets.exp), GTK_POS_RIGHT);
	gtk_scale_set_value_pos(GTK_SCALE(appPtr->localDisplayCtrl.widgets.gain), GTK_POS_RIGHT);
	gtk_widget_set_sensitive(appPtr->localDisplayCtrl.widgets.boxx, FALSE);
	gtk_widget_set_sensitive(appPtr->localDisplayCtrl.widgets.boxy, FALSE);
	gtk_widget_set_sensitive(appPtr->localDisplayCtrl.widgets.exp, FALSE);
	gtk_widget_set_sensitive(appPtr->localDisplayCtrl.widgets.gain, FALSE);
	//pack everything into window
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxgpi), appPtr->localDisplayCtrl.widgets.gpi1);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxgpi), appPtr->localDisplayCtrl.widgets.gpi2);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxgpi), appPtr->localDisplayCtrl.widgets.gpi3);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxgpi), appPtr->localDisplayCtrl.widgets.gpi4);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.boxgpi);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxx), appPtr->localDisplayCtrl.widgets.labelx0);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxx), appPtr->localDisplayCtrl.widgets.x0);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxy), appPtr->localDisplayCtrl.widgets.labely0);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxy), appPtr->localDisplayCtrl.widgets.y0);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxx), appPtr->localDisplayCtrl.widgets.labelcx);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxx), appPtr->localDisplayCtrl.widgets.cx);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxy), appPtr->localDisplayCtrl.widgets.labelcy);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxy), appPtr->localDisplayCtrl.widgets.cy);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.boxx);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.boxy);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.labelexp);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.exp);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.labelgain);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.gain);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.raw);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.show);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.boxmain), appPtr->localDisplayCtrl.widgets.run);
	gtk_container_add(GTK_CONTAINER(appPtr->localDisplayCtrl.widgets.controlWindow), appPtr->localDisplayCtrl.widgets.boxmain);
	//register handlers
	gdk_threads_add_timeout(1000, (GSourceFunc)camTriggerHandler, (gpointer)appPtr); //.localDisplayCtrl.controlWindow);
	...
	g_signal_connect(appPtr->localDisplayCtrl.widgets.run, "toggled",
					 G_CALLBACK(update_run), (gpointer)appPtr); //appPtr->localDisplayCtrl.widgets);
...
gboolean camTriggerHandler(LirenaCaptureApp *app)
{
	...
	if (acquire && app->camState.cameraHandle != INVALID_HANDLE_VALUE)
	{
		...
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->localDisplayCtrl.widgets.gpi1), level);
		xiSetParamInt(app->camState.cameraHandle, XI_PRM_GPI_SELECTOR, 2);
		xiGetParamInt(app->camState.cameraHandle, XI_PRM_GPI_LEVEL, &level);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->localDisplayCtrl.widgets.gpi2), level);
		xiSetParamInt(app->camState.cameraHandle, XI_PRM_GPI_SELECTOR, 3);
		xiGetParamInt(app->camState.cameraHandle, XI_PRM_GPI_LEVEL, &level);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->localDisplayCtrl.widgets.gpi3), level);
		xiSetParamInt(app->camState.cameraHandle, XI_PRM_GPI_SELECTOR, 4);
		xiGetParamInt(app->camState.cameraHandle, XI_PRM_GPI_LEVEL, &level);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->localDisplayCtrl.widgets.gpi4), level);
	}
